  ...
  return (
    <MenuToggleContainer transition={transition} k="key4">
      {!notifications.length ? (
        <h2 className="notification__empty">There is not notifications</h2>
      ) : null}
      {notifications.map((n, i) => (
        <div key={i} className="notification__container">
          <div className="notification__point"></div>
          <span className="notification__text">{n.message}</span>{" "}
          {n.status ? (
            <span className="notification__text">{" (" + n.status + ")"}</span>
          ) : null}
        </div>
      ))}
    </MenuToggleContainer>
...
export const Notifications2: React.FC = () => {
  const { notifications } = useSelector((state: storeState) => state);
  const dispatch = useDispatch();
